...
| 属性名 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `modelValue` | `Array` | - | 组件绑定的数据源。组件内部会基于它生成排序视图，松手后会把最新数组通过 `update:modelValue` 回传给外部。 |
| `column` | `Number` | `0` | 列数。`0` 表示横向流式排列，常用于标签类场景；大于 `0` 表示按固定列数进行单列或宫格布局。比如 `:column="3"` 表示 3 列宫格，`:column="1"` 表示单列列表。 |
| `showClose` | `Boolean` | `false` | 是否启用底部删除条。启用后，拖动到删除条并松手时会删除该项。删除条是组件内置样式，文案会根据 `uni.getAppBaseInfo().appLanguage` 自动适配语言，当前已内置简体中文、繁体中文、英文、拉丁语、法语。 |
| `showAppend` | `Boolean` | `true` | 是否显示 `append` 插槽对应的尾部占位项。这不是九宫格专属属性，只要用了 `append` 插槽，它就是一个通用能力。 |
| `handleMode` | `Boolean` | `false` | 是否启用手柄拖拽模式。当 `handleMode=true` 且提供了 `handle` 插槽时，只能拖手柄开始排序；当 `handleMode=true` 但没有提供 `handle` 插槽时，会自动回退为默认长按拖拽。默认长按模式当前触发时间为 `350ms`。 |
