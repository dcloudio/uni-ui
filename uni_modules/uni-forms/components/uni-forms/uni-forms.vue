...
export default {
	...
	created() {
		...
		this.inputChildrens = [];
		setTimeout(() => {
			this.setRules({ ...this.buildRequiredRules(), ...this.rules });
		}, 0);
	},
	methods: {
		/**
		...
		// 构建子item上带required自动加必填校验
		buildRequiredRules() {
			return this.childrens.reduce((cur, next) => {
				const noticeText =
					typeof next.required === "string" ? next.required : "请填写";

				if (next.required && next.name)
					cur[next.name] = {
						rules: [{ required: true, errorMessage: noticeText }],
					};
				return cur;
			}, {});
		},
