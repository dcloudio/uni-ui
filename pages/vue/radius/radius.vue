<template>
	<view>
		<uni-card :is-shadow="false" is-full>
			<view>使用边框半径辅助样式对元素快速应用 <text class="uni-primary uni-px-3">border-radius</text> 样式。</view>
			<view class="uni-mt-5">规则为 <text class="uni-primary uni-pl-3">uni-radius-${direction}-${size}</text></view>
			<view class="uni-mt-5">如果需要四边，则不需要指定 <text class="uni-primary uni-pl-3">direction</text></view>
			<view class="uni-mt-5">各值的含义请参考文档</view>
		</uni-card>
...
<script>
	export default {
			...
			methods:{
				change(e,type){
					let {direction,size} = this.formData
					this.radiusClass = ''
					direction.forEach(v=>{
