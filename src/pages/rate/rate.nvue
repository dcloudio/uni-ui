<template>
  <view>
    <view class="example-info"><text class="example-title-text">评分组件多用于商品评价打分、服务态度评价、用户满意度等场景。</text></view>
    <uni-title title="基本用法" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :value="2" @change="onChange" />
    </view>
    <uni-title title="设置尺寸大小" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :size="18" :value="5" />
    </view>
    <uni-title title="设置评分数" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :max="10" :value="5" />
    </view>
    <uni-title title="设置星星间隔" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :value="4" :margin="5" />
    </view>
    <uni-title title="设置颜色" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :value="3" color="#bbb" active-color="red" />
    </view>
    <uni-title title="不可点击状态" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :disabled="true" :value="3.5" />
    </view>
    <uni-title title="未选中的星星为镂空状态" type="line"></uni-title>
    <view class="example-body">
      <uni-rate :value="3" :is-fill="false" />
    </view>

  </view>
</template>

<script>
  import uniRate from '@/components/uni-rate/uni-rate.vue'
  import uniTitle from '@/components/uni-title/uni-title.vue'
  export default {
    components: {
      uniRate,
      uniTitle
    },
    data() {
      return {}
    },
    methods: {
      onChange(e) {
        // console.log('rate发生改变:' + JSON.stringify(e))
      }
    }
  }
</script>

<style lang="scss" scoped>
  @import '@/uni.scss';
  @import '@/common/uni-nvue.scss';

</style>
