...
function copyDirByExt(src, dist, exts) {
	if (!fs.existsSync(src)) return
	const paths = fs.readdirSync(src)
	paths.forEach(item => {
		const inputPath = path.join(src, item)
		const outputPath = path.join(dist, item)
		const stat = fs.statSync(inputPath)
		if (stat.isDirectory()) {
			copyDirByExt(inputPath, outputPath, exts)
		} else if (stat.isFile() && exts.includes(path.extname(item))) {
			util.mkdirsSync(path.dirname(outputPath))
			util.copyFile(inputPath, outputPath)
		}
	})
}
